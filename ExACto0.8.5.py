...
def quotedsplit(input, delimiter = ','):  #subroutine for splitting CSV lines (that will often have quotes around them and be separated by commas)  This line can take an optional argument of something other than a comma as a delimiter
    ...
    outputlist = [brokenlist[0]] #initialize a blank list for the output
    del brokenlist[0] #remove the first element of the broken list that we just used to initialize the output list
    outputindex = 0  #initializing the index counter for the output list to 0 (because lists start at 0 for the first element)
