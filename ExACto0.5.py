                    ...
                    if refline and (reflinearray[3] != reference):  #makes sure again that we have some value in refline (we shouldn't be here if we don't) and then checks to make sure that the variant caller called the same reference allele as ExAC has
                        ...
                        if not extraalleles: #otherwise, we continue the analysis
                            ...
                            if not singlenonref and not homozygousrare:  #if this allele is heterozygous for two non-reference alleles
                                ...
                                rarestallele = valuearray[namehash[observedarray[0]][point]['max']]  #if only 1 allele was non-reference, we just use its value for the rarest and don't worry about the combo
                                combomax = 'NA'
                            elif homozygousrare:
                                rarestallele = valuearray[namehash[observedarray[0]][point]['max']]  #and if the person is homozygous for a non-reference allele, the expected frequency of the combination (a homozygote) would be the highest frequency squared
                                combomax = (1/65000)**2
